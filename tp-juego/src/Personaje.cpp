...
Personaje::Personaje() {
  ...
  centrarOrigen(); // centrar el origen del sprite para facilitar el
                   // posicionamiento
  escalarSprite(3.f, 3.f);

  setHitbox(13.f * 2.f,
            16.f *
                2.1f); // Ajustar el tamaño del hitbox según el sprite escalado
...
void Personaje::cargarAtributos(int id, std::string nom, float vida,
                                float armadura, float vel, std::string hab) {
  idPersonaje = id;
  nombre = nom;
  vidaMax = vida;
  vidaActual = vida;
...
Arma &Personaje::getArma() { return armaEquipada; }
